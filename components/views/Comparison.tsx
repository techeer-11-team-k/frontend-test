...
// 전체 아파트 목록 (추가 가능한 아파트들 포함)
const allAvailableAssets: AssetData[] = [
  { 
    id: 1, 
    name: '압구정 현대', 
    region: '강남구', 
...
const initialAssets: AssetData[] = allAvailableAssets.slice(0, 5);
...
const SearchAndSelectApart: React.FC<SearchAndSelectApartProps> = ({ 
    ...
    // 검색 필터링된 아파트 목록
    const filteredAvailableAssets = allAvailableAssets.filter(asset => {
        const isNotAdded = !existingAssets.some(a => a.id === asset.id);
    ...
    // 평형 옵션
    const getPyeongOptions = (assetId: number): PyeongOption[] => {
        const pyeongData: Record<number, PyeongOption[]> = {
            6: [
                { pyeongType: "24평형", area: 84, price: 28.0, jeonse: 13.0 },
                { pyeongType: "32평형", area: 106, price: 32.0, jeonse: 15.0 },
                { pyeongType: "42평형", area: 139, price: 38.0, jeonse: 18.0 },
            ],
            7: [
                { pyeongType: "32평형", area: 106, price: 42.0, jeonse: 19.0 },
                { pyeongType: "34평형", area: 114, price: 48.0, jeonse: 22.0 },
                { pyeongType: "59평형", area: 195, price: 65.0, jeonse: 30.0 },
            ],
            8: [
                { pyeongType: "24평형", area: 84, price: 24.0, jeonse: 12.0 },
                { pyeongType: "32평형", area: 106, price: 28.0, jeonse: 14.0 },
                { pyeongType: "39평형", area: 129, price: 33.0, jeonse: 16.0 },
            ],
        };
        
        return pyeongData[assetId] || [
            { pyeongType: "32평형", area: 106, price: allAvailableAssets.find(a => a.id === assetId)?.price || 0, jeonse: allAvailableAssets.find(a => a.id === assetId)?.jeonse || 0 },
        ];
    };
            ...
            >
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {getPyeongOptions(selectedAssetForPyeong.id).map((pyeongOption, index) => (
                                        <button
                                            key={index}
                                        ...
                                        >
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <p className="text-[13px] text-slate-500 font-medium">
                                                        평당 {(pyeongOption.price / (parseInt(pyeongOption.pyeongType) / 10)).toFixed(2)}억
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-4 pt-3 border-t border-slate-100">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[14px] font-bold text-slate-700">
                                                        {((pyeongOption.jeonse / pyeongOption.price) * 100).toFixed(1)}%
                                                    </span>
                                                </div>
                                            </div>
                                        </button>
                            ...
                            {filteredAvailableAssets.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                                    ...
                                    <p className="text-slate-500 font-medium">
                                        {searchQuery ? '검색 결과가 없습니다' : '추가 가능한 아파트가 없습니다'}
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[13px] text-slate-500 font-medium">
                                                            {asset.region} · {asset.nearestSubway} · {getWalkingTimeRange(asset.walkingTime)}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                ...
                <div className="p-4 border-t border-slate-200 bg-slate-50/50">
                    <p className="text-[13px] text-slate-500 text-center font-medium">
                        최대 8개까지 추가 가능 ({existingAssets.length}/8)
                    </p>
                </div>
            </div>
        </div>
...
export const Comparison: React.FC = () => {
  ...
  const handleSelectAssetForAdd = (asset: AssetData) => {
    // SearchAndSelectApart 컴포넌트 내부에서 처리
  };
  ...
  const handleAddAssetWithPyeong = (asset: AssetData, pyeongOption: PyeongOption) => {
    if (assets.length < 8) {
      const newAsset: AssetData = {
        ...
        pricePerPyeong: Math.round((pyeongOption.price / (parseInt(pyeongOption.pyeongType) / 10)) * 100) / 100,
        pyeongType: pyeongOption.pyeongType,
        area: pyeongOption.area,
        jeonseRate: Math.round((pyeongOption.jeonse / pyeongOption.price) * 1000) / 10,
      };
          ...
          <div className="animate-fade-in space-y-10">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                  ...
                  <div className="divide-y divide-slate-50">
                      <StatRow label="매매가" left="42.5" right="38.2" unit="억" />
                      <StatRow label="전세가" left="19.5" right="18.0" unit="억" />
                      <StatRow label="전세가율" left="45.9" right="47.1" unit="%" />
                      <StatRow label="평당가" left="1.25" right="1.32" unit="억" />
                      ...
                      <StatRow label="역 도보시간" left="5~10분 이내" right="5분 이내" unit="" />
                  </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                  ...
                  <div className="p-6">
                      <div className="grid grid-cols-2 gap-6">
                          {/* Left Side */}
                          <div>
                              <h4 className="text-[15px] font-black text-slate-900 mb-4">래미안 원베일리</h4>
                              <div className="space-y-3">
                                  {schoolTab === 'elementary' ? (
                                      <>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">반포초등학교</span>
                                          </div>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">서래초등학교</span>
                                          </div>
                                      </>
                                  ) : schoolTab === 'middle' ? (
                                      <>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">반포중학교</span>
                                          </div>
                                      ...
                                      <>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">서초고등학교</span>
                                          </div>
                                          <div className="p-3 bg-slate-50 rounded-lg text-center">
                                              <span className="text-[14px] font-medium text-slate-400">-</span>
                                          </div>
                          ...
                          {/* Right Side */}
                          <div>
                              <h4 className="text-[15px] font-black text-slate-900 mb-4">아크로 리버파크</h4>
                              <div className="space-y-3">
                                  {schoolTab === 'elementary' ? (
                                      <>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                      ...
                                      <>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">서래중학교</span>
                                          </div>
                                          <div className="p-3 bg-slate-50 rounded-lg">
                                              <span className="text-[14px] font-bold text-slate-700">반포중학교</span>
                                          </div>
                                              ...
                                              {assets.map((asset) => {
                                                  ...
                                                  return (
                                                      <td key={asset.id} className={`px-6 py-4 text-center text-[15px] tabular-nums ${isFastest || isSlowest ? 'font-black' : 'font-bold'} ${isFastest ? 'text-red-500' : isSlowest ? 'text-blue-500' : 'text-slate-900'}`}>
                                                          {getWalkingTimeRange(asset.walkingTime)}
                                                      </td>
                                                  );
                                              })}
                                          </tr>
                                      ...
                                      >
                                          <div className="flex items-center justify-between">
                                              ...
                                              <div className="text-right">
                                                  ...
                                                  <p className="text-[13px] font-black text-slate-900">{getWalkingTimeRange(closestToSubway.walkingTime)}</p>
                                              </div>
                                          </div>
                                      </div>
                                  </>
